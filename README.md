...
> 주변을 인식하고, 성격에 따라 판단하며, 상황에 반응하는 NPC

Unreal Engine 5에서 VLM의 장면 분석과 LLM의 성격별 시선 계획을
NPC의 시선 행동으로 연결한 개인 졸업 프로젝트입니다.

**바로가기** · [소개](#intro) · [담당 범위](#contribution) · [시스템 구조](#pipeline) · [핵심 구현](#implementation) · [코드 안내](#code-map) · [실행 결과](#results)
...
📄 [관련 논문 보기](docs/AINPC_paper.pdf) · 🏆 한국컴퓨터정보학회 우수논문상

<a id="demo"></a>

### 🎬 실행 모습

**성격별 시선 행동과 실시간 자극 반응**
...
- **중앙집중형 계획 관리**  
  ...
  성격별로 동일 장면을 중복 분석하는 구조를 개선했습니다.
...
<img width="100%" alt="성격별 시선 계획 생성 결과" src="https://github.com/user-attachments/assets/a176d789-0248-4a4b-8aa4-0d3a52cd8d88" />
...
<img width="100%" alt="선택한 객체의 Bounding Box 표시" src="https://github.com/user-attachments/assets/4cb4223f-f770-47fd-8b72-1f3d129252fe" />
...
<details>
<summary><strong>🐍 핵심 코드 보기 · 타깃 위치 추정 결과 공유</strong></summary>

실제 서버 코드에서 발췌·축약했습니다. 데이터 선택과 측정용 인자 등은 생략했습니다.
...
**Eyes → Brain → Body** 순서로 감지·판단·동작을 분리했습니다.
아래 링크는 각 파일의 전체 구현으로 연결됩니다.
...
테스트한 환경에서 시선용 태그를 추가하지 않고도 시선 타깃을 지정할 수 있음을 확인했습니다.
...
> **측정 결과:** 모델 호출 **46.4% 감소** · 총 토큰 **47.0% 감소** · 서버 전체 경과 시간 **47.1% 감소**
